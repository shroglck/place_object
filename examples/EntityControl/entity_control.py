...
def example(pipe, seeds, example_id, global_prompt, entity_prompts):
    # dataset_snapshot_download(dataset_id="DiffSynth-Studio/examples_in_diffsynth", local_dir="./", allow_file_pattern=f"./diffsynth/data/examples/eligen/entity_control/example_{example_id}/*.png")
    masks = [Image.open(f"./diffsynth/data/examples/eligen/entity_control/example_{example_id}/{i}.png").convert('RGB') for i in range(len(entity_prompts[0]))]
    ...
    for seed in seeds:
        ...
        visualize_masks(image, masks, entity_prompts[0], f"eligen_example_{example_id}_mask_{seed}.png")
...
model_manager.load_lora("./models/lightning_logs/version_3/checkpoints/epoch=0-step=1600.ckpt",lora_alpha=1)
...
example(pipe, [10], 1, global_prompt, entity_prompts)
...
example(pipe, [20], 2, global_prompt, entity_prompts)
...
example(pipe, [30], 3, global_prompt, entity_prompts)
...
example(pipe, [40], 4, global_prompt, entity_prompts)
